// ==========================================
  ...
  function loadHomeTempStatus() {
      ...
      runApi('getAllTempData').then(res => {
          if (res) renderHomeTempStatus(res);
      });
  }
          ...
          html += `
            <div class="px-4 py-3 mb-2 bg-white/60 backdrop-blur-md rounded-2xl flex justify-between items-center border border-white/50 shadow-sm">
                <div class="flex items-center gap-2 overflow-hidden flex-1 mr-2">
                     ...
                     <span class="font-bold text-gray-800 text-base truncate">${item.plate}</span>
                </div>
            ...
            <div onclick="openHistoryDetail(${index})" class="bg-white/40 p-4 rounded-2xl mb-2 active:scale-95 transition cursor-pointer hover:bg-white/60 border border-white/50 shadow-sm">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <div class="text-[10px] text-gray-500 flex flex-col gap-1 mt-1">
                            <span class="flex items-center"><i class="fas fa-arrow-circle-right text-green-500 mr-1"></i> ${item.entryDateDisplay} ${item.entryTime}</span>
                            ${!isParked ? `<span class="flex items-center"><i class="fas fa-arrow-circle-left text-red-500 mr-1"></i> ${item.exitDateDisplay} ${item.exitTime}</span>` : ''}
                        </div>
                    </div>
  ...
  function renderTempList() {
      ...
      currentTempList.forEach((item, index) => {
          ...
          container.innerHTML += `
            <div onclick="openTempDetail(${index})" class="bg-white/40 p-4 rounded-2xl mb-2 active:scale-95 transition cursor-pointer hover:bg-white/60 border border-white/50 shadow-sm relative">
                <div class="flex justify-between items-start">
                    <div>
                         <div class="mb-1 flex items-center">
                            <span class="font-bold text-gray-800 text-lg">${item.room}</span>
                            <span class="text-gray-400 mx-2 text-lg">|</span>
                            <span class="font-bold text-gray-800 text-lg">${item.plate}</span>
                         </div>
                        <div class="text-[10px] text-gray-500 flex flex-col gap-1 mt-1">
                            <span class="flex items-center"><i class="fas fa-arrow-circle-right text-green-500 mr-1"></i> ${item.startTimeDisplay}</span>
                            <span class="flex items-center"><i class="fas fa-arrow-circle-left text-red-500 mr-1"></i> ${item.endTimeDisplay}</span>
                        </div>
                    </div>
                    <div class="text-right flex flex-col items-end gap-1">
                        ${badge}
  ...
  function openTempDetail(index) {
      ...
      content.innerHTML = `
        ...
        <div class="w-full aspect-video mt-4 bg-gray-50 rounded-2xl overflow-hidden relative border border-gray-200" onclick="handleTempImgClick()">
            ...
            <div id="detailTempImgOverlay" class="hidden absolute inset-0 bg-black/50 flex items-center justify-center text-white text-xs font-bold backdrop-blur-sm pointer-events-none">
  ...
  function renderAccountingResult(data) { 
      ...
      let html = `
        <div class="overflow-x-auto rounded-lg">
            <table class="w-full text-xs text-left text-gray-800 bg-white/80 rounded-lg overflow-hidden">
